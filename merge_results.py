...
CLASSES_REGEX = ['Email', 'Phone_Number', 'Reference_CEDEX', 'Reference_CS', 'Reference_Code_Postal', 'Social_Network', 'Url']
CLASSES_CRF = ['Location', 'Url', 'Email', 'Phone_Number', 'Reference_CEDEX', 'Reference_CS',
               'Reference_Code_Postal', 'Social_Network', 'Function']
CLASSES_BONZAI = ['Reference_User', 'Project', 'Organization', 'Function']
...
if __name__ == '__main__':
    ...
    for reg_i, crf_i, bonzai_i, bert_i in zip(regex_data, crf_data, bonzai_data, bert_data):
        ...
        for reg_j, crf_j, bonzai_j, bert_j in zip(reg_i["annotations"], crf_i["annotations"], bonzai_i["annotations"], bert_i["annotations"]):
            token = dict()
            ...
            if reg_j["label"] in CLASSES_REGEX:
                ...
                if crf_j['label'] == bonzai_j['label_bonzaiboost']:
                    ...
                    if crf_j['label'] != bert_j['label_bert'] and bert_j['label_bert'] != 'O':
                        votes_nobert += 1
                elif crf_j['label'] == bert_j['label_bert']:
                    token['label'] = crf_j['label']
                    if crf_j['label'] != bonzai_j['label_bonzaiboost']:
                        votes_nobonzai += 1
                elif bonzai_j['label_bonzaiboost'] == bert_j['label_bert']:
