...
OVERSAMPLING_CLASSES = {'Project': 2,
                        'Social_Network': 10,
                        'Reference_User': 20,
                        'Reference_CS': 3}
...
if __name__ == '__main__':
    ...
    if TRAIN:
        ...
        run([
            "bonzaiboost -S " + FOLDER + STEM + " -boost adamh -n 200 --sep '|' -cross k10 -jobs 4 > " + FOLDER + "crossval.log"],
            shell=True)
    ...
    if TEST:
        print("Evaluation")
        with open('dataset/JDC_for_eval.json') as in_f:
            print('Loading', 'JDC_for_eval.json')
            data = json.load(in_f)
